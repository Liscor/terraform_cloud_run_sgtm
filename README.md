...
- The primary MIG is fixed-size (`mig_primary_size`) and on-demand; the overflow
  MIG is Spot (`overflow_spot = true`) and autoscaled. The load balancer fills the
  primary to `max_rate_per_instance` before spilling to the overflow.
- Self-healing: each VM runs the SGTM container under systemd plus a watchdog that
  restarts it if it reports unhealthy; MIG autohealing recreates a VM that stays
  unhealthy. The container health check uses `wget` against `/healthy` — verify the
  SGTM image ships `wget` on first boot (see the note in templates/cloud-init.yaml.tftpl).
...
primary MIG on `update_interval` so instances re-pull the `:stable` image. This
also grants the SGTM service account `roles/compute.instanceAdmin.v1` (needed to
